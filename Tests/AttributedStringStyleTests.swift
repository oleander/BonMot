...
class StringStyleTests: XCTestCase {
    ...
    func testEffectOfAlignmentOnKerning() throws {
        ...
        for (index, rangeToValue) in rangesToValues.enumerated() {
            ...
            let trackingValue = styled.attribute(NSKernAttributeName, at: controlRange.location, effectiveRange: trackingEffectiveRange)
            ...
            if let controlValue = controlValue {
                XCTAssertEqualWithAccuracy(trackingAsNumber!.doubleValue, controlValue, accuracy: 0.0001, line: line)
